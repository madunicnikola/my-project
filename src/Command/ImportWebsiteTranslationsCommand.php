...
class ImportWebsiteTranslationsCommand extends AbstractCommand
{
    protected static $defaultName = 'hoppecke:import:website-translations';
    
    protected function configure()
    {
        $this
            ->setDescription('Import website translations from Excel file');
    }

    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        $io = new SymfonyStyle($input, $output);
        $io->title('Starting website translations import');

        $filePath = PIMCORE_PROJECT_ROOT . '/data-migration/translation_website.xlsx';
        ...
        try {
            ...
            $io->section('Importing website translations...');
            ...
            foreach ($translationsData as $key => $data) {
                try {
                    ...
                    $translationListing->setCondition("`key` = ? AND (`type` = 'message' OR `type` IS NULL OR `type` = '')", [$key]);
                    ...
                    $errorCount++;
                    if ($errorCount === 1 || $output->isVerbose()) {
                        $io->error(sprintf(
                            'Error importing website translation for key "%s": %s',
                            $key,
                            $e->getMessage()
                        ));
            ...
            $io->success(sprintf(
                'Website translations import completed: %d successful, %d failed (total translation keys)',
                $successCount,
                $errorCount
            ));
